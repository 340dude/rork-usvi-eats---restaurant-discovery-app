...
import { useSpecialHours, useAddSpecialHours, useDeleteSpecialHours } from '@/hooks/use-special-hours';
import { confirmAction } from '@/lib/confirm';

// dateString is a plain "YYYY-MM-DD" (no time/timezone). Parsing it directly
// with `new Date(dateString)` reads it as UTC midnight, which then renders as
// the previous day in any timezone behind UTC — build a local date instead.
...
export default function SpecialHoursScreen() {
  ...
  const { data: specialHours = [], isLoading: isLoadingHours, error: hoursError } = useSpecialHours(restaurant?.id);
  const addSpecialHoursMutation = useAddSpecialHours();
  const deleteSpecialHoursMutation = useDeleteSpecialHours();
  ...
  const addSpecialHours = () => {
    ...
    addSpecialHoursMutation.mutate(
      {
        restaurantId: restaurant.id,
        date: newEntry.date,
        reason: newEntry.reason,
        closed: newEntry.closed,
        openTime: newEntry.openTime,
        closeTime: newEntry.closeTime,
      },
      {
        onSuccess: () => {
          setNewEntry({ date: '', reason: '', closed: false, openTime: '', closeTime: '' });
  ...
  const deleteSpecialHours = (id: string) => {
    ...
    confirmAction(
      ...
      () => deleteSpecialHoursMutation.mutate(
        { id, restaurantId: restaurant.id },
        { onError: (err) => Alert.alert('Delete failed', err instanceof Error ? err.message : 'Please try again.') }
      ),
      'Delete'
    );
  };
  ...
  if (isLoadingRestaurant) {
    return (
      <>
        <Stack.Screen options={{ title: 'Special Hours', headerStyle: { backgroundColor: '#00BCD4' }, headerTintColor: '#fff' }} />
  ...
  return (
    <>
      ...
      <ScrollView style={styles.container}>
        ...
        {hoursError && (
          <Text style={[styles.errorText, styles.inlineError]}>Couldn't load special hours.</Text>
        )}
        {/* Add New Entry */}
          ...
          {showAddForm && (
            <View style={styles.addForm}>
              ...
              <View style={styles.formActions}>
                ...
                >
                  {addSpecialHoursMutation.isPending ? (
              ...
              <View key={entry.id} style={styles.entryCard}>
                <View style={styles.entryHeader}>
                  ...
                  <TouchableOpacity
                    ...
                    accessibilityLabel={`Delete special hours entry for ${formatDate(entry.date)}`}
                  >
