...
import { logAnalyticsEvent } from '@/hooks/use-analytics';
...
export default function RestaurantDetailScreen() {
  ...
  const { data: restaurant, isLoading, error, refetch } = useRestaurant(id as string);
  ...
  useEffect(() => {
    if (restaurant) {
      logAnalyticsEvent(restaurant.id, 'view');
    }
  }, [restaurant?.id]);
  ...
  if (error || !restaurant) {
    return (
      <View style={styles.loading}>
        ...
        <Text style={styles.errorTitle}>Couldn't load this restaurant</Text>
  ...
  const handleCall = () => {
    if (restaurant.contact.phone) {
      logAnalyticsEvent(restaurant.id, 'call');
  ...
  const handleDirections = () => {
    logAnalyticsEvent(restaurant.id, 'directions');
  ...
  const handleReportSubmit = () => {
    ...
    submitReport.mutate(
      { restaurantId: restaurant.id, type: reportType, description: reportDescription.trim() },
      {
        onSuccess: () => {
          Alert.alert(
            'Report Submitted',
        ...
        onError: (err) => Alert.alert('Submit failed', err instanceof Error ? err.message : 'Please try again.'),
      }
    );
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.imageContainer}>
        <Image
          source={{ uri: restaurant.images.hero }}
        ...
        <View style={styles.imageOverlay}>
          <TouchableOpacity
            ...
            accessibilityLabel={isFavorite(restaurant.id) ? 'Remove from favorites' : 'Add to favorites'}
          ...
          >
            {restaurant.menu.map((category, index) => (
              <TouchableOpacity
                ...
                accessibilityState={{ selected: selectedMenuCategory === index }}
              >
                <Text style={[
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity onPress={handleReportSubmit} disabled={submitReport.isPending}>
              <Text style={styles.modalSubmit}>{submitReport.isPending ? 'Submitting...' : 'Submit'}</Text>
            </TouchableOpacity>
          </View>
          ...
          <ScrollView style={styles.modalContent}>
            ...
            {reportTypes.map((type) => (
              <TouchableOpacity
                ...
                accessibilityState={{ checked: reportType === type.id }}
              >
                <Text style={[
